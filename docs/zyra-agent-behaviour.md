...
**`create`/`update`/`archive` are staged for review, not applied immediately** (since the 2026-09-02
review-panel change — `applyZyraChatOperations` writes nothing to `testcases`, only to
`ai_generation_requests`, until an explicit `zyraSave`). Zyra still emits the operation the moment it
is confident the user wants it — it must never add its own "would you like me to save these?"
round-trip, the review step already exists downstream and is not Zyra's to gate — but its wording must
say DRAFTED/PROPOSED/staged, never "created"/"saved"/"updated"/"archived" (past tense), since none of
those are true until the user separately reviews and saves the batch.
...
| Provider call never answers at all (stalled connection, silent overload) | Bounded by `ZYRA_ROUTER_TIMEOUT_MS` (60s) / `ZYRA_GENERATE_TIMEOUT_MS` (180s) — generous, not a snappy request timeout, so a call that is genuinely still working is not cut short. The turn is persisted `status: 'timed_out'` with a resume checkpoint rather than answered or silently dropped; the chat shows a Continue action that picks the SAME turn back up (skipping the router call if it already succeeded) via `continueZyraChatMessage`, instead of re-asking the user or silently repeating the whole request. |
...
| 2026-09-03 | The system prompt's "there is no draft buffer, `create` writes immediately" instruction (and this doc's §4) corrected to describe the real, current behaviour: `create`/`update`/`archive` are staged for review and write nothing until `zyraSave`. The model is still told to emit the operation immediately rather than ask its own "would you like me to save these?" — only the completion-tense wording changed. | The 2026-09-02 "chat behaviour" change added the review-panel staging (`ai_generation_requests` + `zyraSave`) but never updated the prompt or this doc, both of which still told the model the opposite. The model, trusting its own instructions, described staged drafts as already created/saved — reported as "Zyra says the test cases have been generated, but no test cases are actually created." `reconcileZyraReply`'s reviewHint footer was already correcting for this after the fact; this fixes it at the source instead of relying only on the footer. |
| 2026-09-03 | The transcript now also flags an `answer`-routed turn as an implicit offer awaiting confirmation when its own words end in "would you like me to / should I / shall I <act on testcases>?" — alongside the existing PROPOSAL annotation, which only ever fired for a turn routed create/archive/update. | A gap-analysis turn correctly routes `answer` (it changed nothing), so it never got an antecedent flag the way an archive confirmation does — reported as "Zyra doesn't respond after the user confirms generation for identified gaps" (the router had no structural signal, network log showed the request completed with no error, just no action). The model was always instructed to resolve such confirmations from context; this adds the same structural nudge the create/archive/update case already has, without a keyword router deciding what "yes" means. |
| 2026-09-03 | Every outbound `fetch()` to an AI provider in the Zyra chat and task-generation paths now carries a timeout (`ZYRA_ROUTER_TIMEOUT_MS` 60s, `ZYRA_GENERATE_TIMEOUT_MS` 180s — generous, not a snappy request timeout). A timeout is not treated as a failure: the turn is persisted `status: 'timed_out'` with a `resume_checkpoint`, and `POST .../messages/:messageId/continue` (`continueZyraChatMessage`) picks the SAME turn back up — skipping the router call if it had already resolved a suite/count before the drafting call stalled — rather than repeating the whole request or asking the user to re-explain. Claiming a checkpoint is one atomic `UPDATE … WHERE status = 'timed_out'`, so a double-click or two tabs can only have one request win; a genuinely new message in the session expires any dangling checkpoint so Continue can never resolve to superseded context. | No call in this chain — not the outbound fetch, not the frontend's `api()` helper, not nginx (24h `proxy_read_timeout`) — had ever carried a timeout. A provider that stalled left the request open indefinitely: no reply, no error, the chat's "thinking" state stuck forever — reachable from the same "Zyra doesn't respond" report as the offer-annotation entry above, and the one failure mode nothing else in this file already turns into a visible outcome. |
| 2026-09-08 | Two fixes to the "did this turn actually persist anything" checks: (1) `ZYRA_COMPLETION_CLAIM`/`ZYRA_PERSISTED_CLAIM` gained a negative lookbehind excluding continuous/future/modal auxiliaries ("being", "getting", "will be", "should be", "not yet"...) immediately before the verb, so a reply honestly describing staged-not-yet-saved work ("Cases being updated: PRO-TC-239…") no longer trips the same match as "Created 7 test cases" did; `ZYRA_ALREADY_DISCLOSED` now also recognises the present-tense staging disclosure §4 tells the model to use ("nothing IS changed…", "…staged for review", "…until you save them"), not only the past-tense "nothing WAS saved". (2) `zyraTranscript`'s "saved N testcase(s)" note and `zyraPendingConfirmation`'s PROPOSAL/OFFER detection now share one `zyraRowPersisted` predicate keyed on the row's `action` (`proposed-*`/`suggested` = not yet written; anything else with an id = real) instead of each independently checking raw `id` presence — a staged `update`/`archive` preview carries the id of the EXISTING testcase it previews changing (`applyZyraChatOperations`'s `preview = {...row, ...fields}`), which the old per-call-site `id`-presence check misread as "already saved" for one caller while the other correctly called it a PROPOSAL, producing a single transcript line that simultaneously said `[saved 2 testcase(s)...]` and `[...PROPOSAL still awaiting the user's go-ahead]`. Also extended `ZYRA_OFFER_PATTERN`'s coverage (new `ZYRA_STAGED_AWAITING_PATTERN`) to a declarative staged-for-review summary that never phrases itself as a "would you like me to...?" question. | Reported directly: "Review existing test cases and rewrite any weak or vague expected result" got the false "⚠️ Sorry! Nothing was saved" banner on a reply that was already honest about staging, and every follow-up "go ahead" repeated the same banner — the update proposal's real-id preview rows defeated both `zyraPendingConfirmation` (no PROPOSAL annotation → the confirmation-retry in `sendZyraChatMessage` never fired) and, independently, the completion-claim regexes (no tense check → "being updated" matched like "was updated"), so the user's confirmation had no antecedent and the turn just repeated the same non-committal answer. No BetterBugs session; found by architecture audit + `zyra-reply-guards.spec.ts`. |
| 2026-09-08 | `move_to_suite ... fromLastPlan=true` — the documented way to handle "save them to `<suite>`" right after a generation — is fixed for a batch that is still only staged. `zyra_chat_sessions.last_completed_plan` now also carries `pendingReviewRequestIds`: `applyZyraChatOperations` records a create batch's `ai_generation_requests` id there the moment it stages it (`recordZyraPendingReviewRequest`), `zyraPendingCreateBatches` re-verifies each one is still `task_status = 'in_review'` before trusting it, and a `move_to_suite fromLastPlan` op now patches the still-pending draft's `draft.suiteId` directly in `generated_payload` (never auto-saving it) in addition to — not instead of — the existing real-row move for an already-saved batch. `zyraSaveAttempt` now records a newly-created row's real id into `last_completed_plan.testcaseIds` at the one point it actually becomes real, and drops the batch out of `pendingReviewRequestIds` once nothing of it is left staged. The system prompt's "Most recently generated batch" section and its `fromLastPlan` instruction were corrected to describe both cases (saved and still-staged) instead of unconditionally saying "already saved to the repository". | Found by architecture audit while fixing the entry above, not reported: since the 2026-09-03 staging change, a staged `create`'s `applied.testcases[].id` is always null (nothing is written until `zyraSave`), so the code that populated `last_completed_plan` from that array — in both `sendZyraChatMessage` and the batch-plan loop — always wrote an empty `testcaseIds`, and `sendZyraChatMessage`'s version did this UNCONDITIONALLY, silently erasing `pendingReviewRequestIds` moments after `applyZyraChatOperations` set it in the very same call. Net effect: `fromLastPlan` always resolved to zero targets, so the mainline "generate, then say which suite" flow the prompt itself instructs the model to use always answered "the requested testcases do not exist in this project yet" — false, since they existed as unsaved drafts. Zero test coverage anywhere (unit or e2e) had ever exercised `fromLastPlan`; `e2e/api/zyra-chat-consistency.spec.ts` (ZCC-B-04, ZCC-B-05) now does, against the real staged-draft and real-DB-move cases respectively. |
| 2026-09-08 | Every read-modify-write of `last_completed_plan` / a batch's `generated_payload` from the fix above now takes the same row lock `zyraSaveAttempt` already used: `recordZyraPendingReviewRequest` locks `zyra_chat_sessions` (`SELECT ... FOR UPDATE`) inside a transaction instead of a bare `SELECT` + `UPDATE`, and the `move_to_suite fromLastPlan` patch was split into `zyraPendingCreateBatches` (an unlocked candidate list only) plus a new `patchZyraPendingBatchSuite` that locks the specific `ai_generation_requests` row before reading and rewriting its `generated_payload`. | Follow-up self-review, same day: `zyraSaveAttempt` (the review panel's Save button) is a separate HTTP request, not covered by `sendZyraChatMessage`'s per-session `processing_since` claim, so it can run concurrently with a chat "save them to `<suite>`" turn on the same batch. The unlocked version blind-overwrote `generated_payload` with whatever it had read first — if a concurrent Save committed a shorter (already-saved-entries-removed) array in between, this would silently resurrect an already-saved draft back into the pending payload, and the next Save would create a **second, duplicate real testcase** from it. No report; found by re-auditing this same fix before it shipped. |
| 2026-09-08 | `moveSuites` (which drives `reconcileZyraReply`'s "📦 Moved to suites (actual)" footer) is now registered for a `move_to_suite` op's target suite only when a real DB move happened (`targets.length`), or when NOTHING matched either path at all — never when the suite was touched only via the pending-draft patch above. | Follow-up self-review, same day: the footer's suite registration was still unconditional, so a `fromLastPlan` turn that only repointed a still-pending draft's suite (no real row to move) rendered "Suite: 0 (none matched)" in the same reply as reviewHint's true "N of them are staged for your review... will be filed into Suite" — a self-contradiction of exactly the shape the 2026-09-02 moveBreakdown entry above exists to prevent, reintroduced from a new angle by this same day's fix. `e2e/api/zyra-chat-consistency.spec.ts` ZCC-B-04 now asserts the footer is absent from a pending-only match. No report; found by re-auditing this same fix before it shipped. |
