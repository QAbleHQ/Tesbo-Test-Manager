...
public final class WorkspaceService {
    ...
    public static Optional<Map<String, Object>> getCurrentUserWorkspace(UUID userId) {
        String sql = "SELECT o.id, o.name, o.slug, o.created_at " +
    ...
    public static List<Map<String, Object>> listWorkspaceMembers(UUID orgId, UUID actorId) {
        ...
        String sql = "SELECT u.id, u.email, u.name, om.role, om.created_at " +
        ...
        try (Connection c = Database.getDataSource().getConnection();
            ...
            while (rs.next()) {
                out.add(Map.of(
                        ...
                        "role", rs.getString("om.role"),
                        "joinedAt", rs.getTimestamp("om.created_at").toInstant().toString()
                ));
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    /** Add workspace member by email: finds user by email and adds them. Fails if no user with that email. */
    public static void addWorkspaceMemberByEmail(UUID orgId, UUID actorId, String email, String role) {
        ...
        if (targetUserId.isEmpty()) {
            throw new io.javalin.http.BadRequestResponse("No user found with this email. They must sign in once to be added.");
        }
