...
public final class AutomationScriptBuilderService {
    ...
    public static String buildStagehandScript(String testName, List<Map<String, Object>> events) {
        String sanitizedName = (testName == null || testName.isBlank()) ? "generated stagehand automation test" : testName.trim();
        StringBuilder sb = new StringBuilder();
        sb.append("// @automation-language: stagehand-ts\n");
        sb.append("// Generated by Aegis (Stagehand mode)\n");
        sb.append("// Runtime context provides: stagehand, page, agent, z, assert\n\n");
        sb.append("// Test: ").append(escape(sanitizedName)).append("\n");
        sb.append("const page = stagehand.context.pages()[0];\n");
        sb.append("const agent = stagehand.agent();\n\n");
        ...
        for (Map<String, Object> action : finalizedActions) {
            ...
            if ("navigate".equals(type)) {
                String url = asText(action.get("url"));
                if (!url.isBlank()) {
                    sb.append("await page.goto(").append(quote(url)).append(");\n");
                    wroteAction = true;
                }
                continue;
            }
            if ("click".equals(type) || "type".equals(type)) {
                String instruction = stagehandInstructionForAction(action);
                if (!instruction.isBlank()) {
                    sb.append("await stagehand.act(").append(quote(instruction)).append(");\n");
                    wroteAction = true;
                }
                continue;
            }
            if ("assert_visible".equals(type) || "assert_clickable".equals(type) || "assert_text".equals(type)) {
                String assertionPrompt = stagehandAssertionPrompt(action);
                if (!assertionPrompt.isBlank()) {
                    sb.append("const assertionResult = await stagehand.extract(\n");
                    sb.append("  ").append(quote(assertionPrompt)).append(",\n");
                    sb.append("  z.object({ pass: z.boolean(), reason: z.string().optional() })\n");
                    sb.append(");\n");
                    sb.append("assert(assertionResult.pass, assertionResult.reason || ").append(quote("Assertion failed")).append(");\n");
                    wroteAssertion = true;
                }
            }
        }
        ...
        if (!wroteAction) {
            sb.append("await agent.execute(").append(quote("Complete the testcase objective and validate expected outcomes.")).append(");\n");
        }
        if (!wroteAssertion) {
            sb.append("const smokeCheck = await stagehand.extract(\n");
            sb.append("  ").append(quote("Verify the page reached a valid post-condition for this test. Return pass=true only if outcome is successful.")).append(",\n");
            sb.append("  z.object({ pass: z.boolean(), reason: z.string().optional() })\n");
            sb.append(");\n");
            sb.append("assert(smokeCheck.pass, smokeCheck.reason || ").append(quote("Final validation failed")).append(");\n");
        }
    ...
    private static List<Map<String, Object>> collectFinalizedActions(List<Map<String, Object>> events) {
        ...
        for (Map<String, Object> event : events) {
            ...
            if ("command_executed".equals(type)) {
                ...
                List<Map<String, Object>> stagehandActions = asListOfMaps(execution == null ? null : execution.get("stagehandActions"));
                if (!stagehandActions.isEmpty()) {
    ...
    private static void collectStepsFromStagehandActions(
            List<Map<String, Object>> out,
            List<Map<String, Object>> stagehandActions,
            String commandStatus
    ) {
        if (stagehandActions == null || stagehandActions.isEmpty()) return;
        ...
        for (Map<String, Object> entry : stagehandActions) {
    ...
    private static String stagehandInstructionForAction(Map<String, Object> action) {
        String type = asText(action.get("action")).toLowerCase();
    ...
    private static String stagehandAssertionPrompt(Map<String, Object> action) {
        String type = asText(action.get("action")).toLowerCase();
        String target = asText(action.get("targetDescription"));
