...
test.describe("profile", () => {
  test("PATCH /api/auth/me updates the name and mobile number, and it persists", { tag: '@tesbo.testId("TES-TC-1402")' }, async ({ playwright }) => {
    const anon = await anonContext(playwright);
    ...
    const newName = `E2E API Profile ${Date.now()}`;
    ...
    const patchRes = await anon.patch("/api/auth/me", { data: { name: newName, mobileNumber: newMobileNumber } });
  ...
  test("PATCH /api/auth/me rejects an empty name and an out-of-range mobile number", { tag: '@tesbo.testId("TES-TC-1403")' }, async ({ playwright }) => {
    const anon = await anonContext(playwright);
    ...
    const emptyNameRes = await anon.patch("/api/auth/me", { data: { name: "   " }, failOnStatusCode: false });
  ...
  test("an unauthenticated PATCH /api/auth/me is rejected", { tag: '@tesbo.testId("TES-TC-1404")' }, async ({ playwright }) => {
    const anon = await anonContext(playwright);
    const res = await anon.patch("/api/auth/me", { data: { name: "Nope" }, failOnStatusCode: false });
    expect(res.status()).toBe(401);
    await anon.dispose();
  });
});
