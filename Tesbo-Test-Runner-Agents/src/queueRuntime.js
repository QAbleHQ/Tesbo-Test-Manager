...
export async function enqueueExecutionJob(payload) {
  ...
  const job = await queue.add("execution", payload, executionJobOpts(payload));
  return { queueJobId: String(job.id), state: "queued" };
}

export async function enqueueExecutionJobsBatch(payloads) {
  const queue = ensureQueue();
  if (!payloads.length) {
    return [];
  }
  const bulk = payloads.map((payload) => ({
    name: "execution",
    data: payload,
    opts: executionJobOpts(payload),
  }));
  const jobs = await queue.addBulk(bulk);
  logInfo("queue_batch_enqueued", { count: jobs.length });
  return jobs.map((job) => ({
