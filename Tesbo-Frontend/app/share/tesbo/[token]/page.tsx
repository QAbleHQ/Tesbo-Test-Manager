...
export default function PublicTesboRunPage() {
  ...
  useEffect(() => {
    ...
    setOpenSpecs((prev) => {
      ...
      groupedCases.forEach((group) => {
        if (next[group.specName] === undefined) next[group.specName] = true;
      });
      return next;
    });
  }, [groupedCases, run]);
  ...
  if (loading) {
    return (
      <main className="max-w-6xl mx-auto px-6 py-8">
        <p className="text-sm text-[var(--muted)]">Loading shared report...</p>
      </main>
    );
  }
  ...
  if (error || !run) {
    return (
      <main className="max-w-6xl mx-auto px-6 py-8">
        <p className="text-sm text-rose-600">{error || "Shared report not available."}</p>
      </main>
    );
  }
  ...
  return (
    <main className="max-w-7xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="rounded-full border border-[var(--border)] bg-[var(--surface-secondary)] px-3 py-1 text-xs font-medium text-[var(--muted)]">
